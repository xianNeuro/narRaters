...
class TutorialPDF(FPDF):
    ...
    def render_toc(self, toc_page_number):
...
def build_pdf():
    ...
    pdf.add_page()
    # Reserve enough space: two blank pages for TOC
    pdf.add_page()
    ...
    pdf.body_text(
        "Click any row in the dashboard to open its detail view. The detail view "
        "has tabs for each processing step. Each tab shows the input on the left "
        "and the editable output on the right."
    )
    pdf.add_screenshot("05_story_detail.png",
                       "Figure 5: Story detail view -- left: input data; right: editable output")
    pdf.add_screenshot("07_subject_detail.png",
                       "Figure 6: Subject detail view with tabs for each processing step")
